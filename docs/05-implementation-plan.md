...
- [ ] El missatge renderitzat **conté l'acrònim del pla**: cap valor del missatge no és un nom pelat sense qualificar, perquè el blueprint no defineix cap `variables:` i un `{{ acronym }}` sense `trigger.event.data.` rendiria una cadena buida sense petar (§5.2 de [`03`](03-feature-spec.md))
