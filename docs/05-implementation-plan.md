...
- [ ] **Cap error de plantilla en el missatge** amb cap de les quatre fases que un event pot portar (`prealerta`, `alerta`, `emergencia`, `unrecognized`): un test les renderitza totes. El guard de `unrecognized` va **abans** de qualsevol `index()`, que és el que ho garanteix
